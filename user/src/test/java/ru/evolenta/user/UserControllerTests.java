...
class UserControllerTests {
	...
	void testGetUserByUsernameSuccess() throws Exception {
		...
		User targetUser = users.stream().filter(user -> user.getUsername().equals(targetUsername)).findFirst().get();
		Mockito.when(this.userService.getUserByUsername(targetUsername))
				.thenReturn(ResponseEntity.ok(targetUser));
